...
def show_geo_view(url, json_file, file, background_id, error_message_list):
    ...
    if origin_location is not None:
        ...
        try:
            ...
            geo_view_path = ADMIN_FRONT_HTML_PATH + str(file) + ".html"
...
def show_data_statis(url, file, error_message_list):
    ...
    for files in os.listdir(file_path):
        if files.count('dyna') > 0:
            ...
            data = pd.read_csv(DATASET_PATH + file + os.sep + files, index_col='dyna_id')
            ...
            data = pd.read_csv(DATASET_PATH + file + '/' + files, index_col='dyna_id')
            ...
            try:
                data = dd.read_csv(DATASET_PATH + file + '/' + files)
                # test_dict = {'id': [], 'inflow': [], 'outflow': [], 'abs_flow': []}
                if 'flow' in data:
                ...
                logger.error("{} gridod 文件读取错误，异常信息：{}", DATASET_PATH + file + '/' + files, ex)
                continue
    ...
    if name2 is None:
        pic = (Scatter()
            ...
            toolbox_opts=opts.ToolboxOpts(
               ...
               .render(ADMIN_FRONT_HTML_PATH + str(file) + ".html"))
    else:
        pic = (Scatter()
               .add_xaxis(asix_x)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        func = sys.argv[1]
        if func=='get_geo_json':
            dataset=sys.argv[2]
            save_path=sys.argv[3]
            error_message_list=[]
            status=get_geo_json(dataset,save_path,error_message_list)
            status_path=save_path.split('_geo_json')[0]+'/status'
            with open(status_path, 'w') as f:
                f.write(str(status))
            print(status)
        elif func=='transfer_geo_json':
            url=sys.argv[2]
            file_name=sys.argv[3]
            background_id=int(sys.argv[4])
            error_message_list=[]
            status=transfer_geo_json(url,file_name,background_id,error_message_list)
            status_path=url.split('_geo_json')[0]+'/status'
            with open(status_path, 'w') as f:
                f.write(str(status))
            print(status)
    else:
